...
def split_post_by_chunks(full_post: str, chunk_size: int) -> List[str]:
    chunks = []
    for i in range(0, len(full_post), chunk_size):
        chunks.append(full_post[i:i + chunk_size])
    return chunks
...
async def search_arxiv_last_n_by_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for section in post_sections:
        post_sections_chunks.extend(split_post_by_chunks(section, chunk_size=CHUNK_SIZE))
